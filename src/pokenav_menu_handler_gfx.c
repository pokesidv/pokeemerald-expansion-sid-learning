...
static u32 LoopedTask_CannotAccessDexNav(s32);
...
static void PrintCannotAccessDexNav(void);
...
static const LoopedTask sMenuHandlerLoopTaskFuncs[] =
{
    ...
    [POKENAV_MENU_FUNC_CANNOT_ACCESS_DEXNAV]  = LoopedTask_CannotAccessDexNav
};
...
static const u16 sOptionsLabelGfx_AccessDexNav[]  = {0x0A0, PALTAG_OPTIONS_BEIGE - PALTAG_OPTIONS_START};
static const u16 sOptionsLabelGfx_AccessPokedex[] = {0x0C0, PALTAG_OPTIONS_RED - PALTAG_OPTIONS_START};
...
static u32 LoopedTask_CannotAccessDexNav(s32 state)
{
    switch (state)
    {
    case 0:
        PlaySE(SE_FAILURE);
        PrintCannotAccessDexNav();
        return LT_INC_AND_PAUSE;
    case 1:
        if (IsDma3ManagerBusyWithBgCopy())
            return LT_PAUSE;
...
{
    struct Pokenav_MenuGfx * gfx = GetSubstructPtr(POKENAV_SUBSTRUCT_MENU_GFX);
    const u8 * s = gText_Pokenav_Cannot_Access_DexNav;
    u32 width = GetStringWidth(FONT_NORMAL, s, -1);
    FillWindowPixelBuffer(gfx->optionDescWindowId, PIXEL_FILL(6));
    AddTextPrinterParameterized3(gfx->optionDescWindowId, FONT_NORMAL, (192 - width) / 2, 1, sOptionDescTextColors2, 0, s);
}
