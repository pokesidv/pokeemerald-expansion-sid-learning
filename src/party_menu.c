...
static void CreateCancelConfirmWindows(bool8 chooseHalf)
{
    ...
    if (gPartyMenu.menuType != PARTY_MENU_TYPE_MULTI_SHOWCASE)
    {
        ...
        {
            mainOffset = GetStringCenterAlignXOffset(FONT_SMALL, gText_Cancel, 43);
            AddTextPrinterParameterized3(cancelWindowId, FONT_SMALL, mainOffset + offset, 0, sFontColorTable[0], TEXT_SKIP_DRAW, gText_Cancel);
        }
        else
        {
            mainOffset = GetStringCenterAlignXOffset(FONT_SMALL, gText_Cancel2, 43);
            AddTextPrinterParameterized3(cancelWindowId, FONT_SMALL, mainOffset + offset, 0, sFontColorTable[0], TEXT_SKIP_DRAW, gText_Cancel2);
        }
